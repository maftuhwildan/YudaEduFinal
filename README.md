...
| **Rate Limiting** | Maksimal 5 percobaan login per username per 60 detik |
| **Secure Cookies** | HTTP-only, secure, sameSite cookie untuk sesi |
...
| **Authentication** | Custom (bcrypt + HTTP-only cookie sessions) |
...
# Password admin awal saat seed (opsional, default: admin123)
ADMIN_INITIAL_PASSWORD="your-secure-password"

# Google GenAI API Key (opsional, untuk AI question generation)
GOOGLE_GENAI_API_KEY="your-google-genai-api-key"

# Groq API Key (opsional, alternatif AI provider)
GROQ_API_KEY="your-groq-api-key"
```

> [!TIP]
> Untuk production, tambahkan `?connection_limit=10` di akhir `DATABASE_URL`
...
| `ADMIN_INITIAL_PASSWORD` | Password admin awal saat seed | ❌ (default: `admin123`) |
...
# Seeding
node prisma/seed-admin.js # Seed akun admin awal
```
...
4. **Seed akun admin (jika pertama kali):**
   ```bash
   node prisma/seed-admin.js
   ```
